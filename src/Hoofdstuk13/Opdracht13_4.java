...
public class Opdracht13_4 extends Applet {
    ...
    Color color = null;
    Button redBrk = new Button("Rode bakstenen");
    Button grayB = new Button("Grijze betonblokken");
    int sz = 0;

    public void init() {
        setSize(1920, 953);
        add(redBrk);
        redBrk.addActionListener(new RedListener());
        add(grayB);
        grayB.addActionListener(new GrayListener());
    }
    ...
    void buildWall(Graphics g) {
        ...
        for (int iRow = 0; 2 + 22 * iRow < getHeight(); iRow++) {
            for (int iCol = 0; 2 + 52 * (iCol - 1) < getWidth(); iCol++) {
                if (iRow % 2 == 0) {
                    g.fillRect(sz * (2 + 52 * iCol), sz * (2 + 22 * iRow),
                            sz * 50, sz * 20);
                } else {
                    g.fillRect(sz * (-25 + 52 * iCol), sz * (2 + 22 * iRow),
                            sz * 50, sz * 20);
                }
            }
        }
